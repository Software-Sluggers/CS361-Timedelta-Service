...
def test_add_days(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="add", value=5, unit="days"
    )
    ...
    assert (
        body["Summary"]
        == "5 days after 2026-08-02T12:00:00.000Z is 2026-08-07T12:00:00.000Z"
    )


def test_add_hours(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="add", value=3, unit="hours"
    )
...
def test_add_minutes(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="add", value=30, unit="minutes"
    )
...
def test_add_seconds(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="add", value=45, unit="seconds"
    )
...
def test_negative_value(get_timedelta):
    ...
    assert (
        body["Summary"]
        == "5 days before 2026-08-02T12:00:00.000Z is 2026-07-28T12:00:00.000Z"
    )
...
def test_invalid_timestamp(get_timedelta):
    status, body = get_timedelta(
        timestamp="not-a-timestamp", operation="add", value=5, unit="days"
    )
...
def test_invalid_operation(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="subtract", value=5, unit="days"
    )
...
def test_invalid_unit(get_timedelta):
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000Z", operation="add", value=5, unit="weeks"
    )
...
def test_timestamp_case_sensitivity(get_timedelta):
    ...
    status, body = get_timedelta(
        timestamp="2026-08-02T12:00:00.000z", operation="add", value=1, unit="days"
    )
